...
M, input_size, hidden_size, output_size = 4, 2, 100, 3

# 生成随机数当作样本，同时用Variable 来包装这些数据，设置 requires_grad=False 表示在方向传播的时候，
# 我们不需要求这几个 Variable 的导数
df_train = pd.read_csv('newdis/train.csv')
X_train = df_train[['x', 'y']].values
y_train = df_train[['r_x','r_y','r_z']].values
...
y_pred_t = model(x)
y_pred_t = scalar.inverse_transform(y_pred_t.detach().numpy())
y_r = scalar.inverse_transform(y)
#loss2 = loss_fn(y_pred, y_r)
loss2 = np.abs(y_pred_t - y_r)
sum1 = np.sum(loss2)*0.0254/258

loss_fn2 =  nn.MSELoss(reduction='sum')
#loss2 = loss_fn2(y_pred_t, y_r)
df_test = pd.read_csv('newdis/test.csv')
X_test = df_test[['x', 'y']].values
y_test = df_test[['r_x','r_y','r_z']].values
...
loss3 = np.abs(y_pred_test - y_test)
sum2 = np.sum(loss3)*0.0254/22
s=0
s1=0
for i in range(len(y_pred_t)):
    s =  s+ np.sqrt((y_pred_t[i][0] - y_r[i][0])**2+(y_pred_t[i][1] - y_r[i][1])**2+(y_pred_t[i][2] - y_r[i][2])**2)

for i in range(len(y_test)):
    s1 =  s1+ np.sqrt((y_pred_test[i][0] - y_test[i][0])**2+(y_pred_test[i][1] - y_test[i][1])**2+(y_pred_test[i][2] - y_test[i][2])**2)
#loss3 = loss_fn2(y_pred_test, y_testt)
#print("loss2: "+loss2)
print("loss2:"+str(s*0.0254/len(y_pred)))
print("loss3:"+str(s1*0.0254/len(y_test)))
